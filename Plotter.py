...
class Plotter(Plot_base):
    def __init__(self, size=(7, 7), title="Path Planning Problem for an Swarm"):
        super().__init__(size, title)
        self.swarm_scatter = None
        self.goal_scatter = None
        self.edge_collection = None
        self.region_patch = None

    def init_scene(self, obstacles, swarm):

        self.clear()

        # draw map ONCE
        self.show_map(obstacles=obstacles, plot_title="Swarm")

        self.set_equal()
        self.show_grid()

        # 🔵 swarm points
        pts = np.array([r.coordinate for r in swarm.followers])
        self.swarm_scatter = self.ax.scatter(
            pts[:, 0], pts[:, 1],
            c='blue', s=20
        )

        # 🔴 goal
        gx, gy = swarm.center.goal
        self.goal_scatter = self.ax.scatter(
            gx, gy, c='red', marker='*', s=100
        )

        # ⚫ edges (init empty)
        self.edge_collection = LineCollection([], colors='black', linewidths=0.5)
        self.ax.add_collection(self.edge_collection)
        self.region_patch = None

    def update_virtual_region(self, center, shape, params):

        x, y = center

        if self.region_patch is None:
            # create once
            if shape == "CIRCLE":
                self.region_patch = patches.Circle((x, y), params['R'], fill=False)
            else:
                self.region_patch = patches.Ellipse((x, y), 1, 1, fill=False)

            self.ax.add_patch(self.region_patch)

        # update instead of recreate
        if shape == "CIRCLE":
            self.region_patch.center = (x, y)
            self.region_patch.radius = params['R']

        elif shape == "ELLIPSE":
            self.region_patch.center = (x, y)
            self.region_patch.width = 2 * params['a']
            self.region_patch.height = 2 * params['b']
            self.region_patch.angle = np.degrees(params['angle'])
        
    # UPDATE SWARM 
    def update_swarm(self, swarm):
        pts = np.array([r.coordinate for r in swarm.followers])
        self.swarm_scatter.set_offsets(pts)


    # UPDATE EDGES
    def update_edges(self, edges):
        # edges = [ (p1, p2), ... ]
        lines = [ [p1, p2] for (p1, p2) in edges ]
        self.edge_collection.set_segments(lines)
